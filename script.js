...
// Creating a loop to create the 0-9 key
...
// Creating the minus key seperate since it was breaking a loop
function minusKey() {
	document.querySelector(`#calculator`).innerHTML += `<button id="minus-key">-</button>`;
}

// Calling the minusKey function to get key displayed on the page
minusKey();

// Creating a function to update the display with the user key inputs
function keyPress(inputKey) {
	document.querySelector(`#calc-output`).value += inputKey;
}

// Creating a function to do the math
function doMath() {
...
// Creating the listener using a loop to get 
